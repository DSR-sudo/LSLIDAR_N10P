...
{
// 输入层只负责把来源转换成 LslidarPacket，不解析型号协议。
// driver/polling 模块随后完成帧头、固定长度/动态长度和 CRC 判断。
...
public:
  ...
  void UDP_difop();
protected:
	// 连接参数由基类统一读取，socket 和 PCAP 子类只关注各自的读取机制。
...
private:
	// 设备 IP 用于过滤同一网卡上其他 UDP 来源；DIFOP 地址只用于控制帧。
  in_addr devip_;
  in_addr devip_difop;
...
private:
	// empty_ 区分第一次打开失败和循环回放结束；read_once_ 控制是否自动重开文件。
